...
router.delete("/users/:id", ensureAuthed, ensureAdmin, async (req, res) => {
  try {
    ...
    if (req.user && String(req.user._id || req.user.id) === String(id)) {
      // optional: render with a message; we redirect for simplicity
    ...
    // you may want to clear references in other collections (favourites, etc.)
    // e.g. await School.updateMany({}, { $pull: { favourites: id } });

    return res.redirect("/admin/users?msg=user_deleted");
  } catch (e) {
    console.error("[admin/users/delete] error:", e);
    return res.status(500).redirect("/admin/users?error=delete_failed");
  }
});
