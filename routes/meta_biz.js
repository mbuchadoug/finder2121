...
async function sendMenuForUser(res, biz, providerId) {
  ...
  const baseMenu = MENU_SCHEMA[role] || MENU_SCHEMA.owner;
  const filteredMenu = filterMenuByPackage(baseMenu, biz);

  const { text, map } = renderMenuText(filteredMenu);

  // 🔐 store menu map for numeric routing
  biz.sessionData = biz.sessionData || {};
  biz.sessionData.menuMap = map;
  await saveBiz(biz);
...
if ((state === "idle" || state === "ready") && isSingleNumber && !state.startsWith("report_")) {
  ...
  const choice = biz.sessionData?.menuMap?.[trimmed];
